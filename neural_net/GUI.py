...
class Visualizer:
    ...
    def paint(self, e):
        ...
        if self.last_xy is not None:
            ...
            self.draw_canvas.create_line(x_prev, y_prev, e.x, e.y, fill="white", width=r * 2, capstyle=tk.ROUND, smooth=True)
            self.draw.line([x_prev, y_prev, e.x, e.y], fill=255, width=r * 2)
        self.draw_canvas.create_oval(e.x - r, e.y - r, e.x + r, e.y + r, fill="white", outline="white")
    ...
    def act_color(self, maxes):
        if maxes.max() > 1:
            return NODE_ACTIVE_COLOR
        else:
            return NODE_COLOR


    def check_Max(self, activations):
        maxes = []
        for i in range(len(activations)):
            maxes.append(np.asarray(activations[i]).max())
        return np.array(maxes)


    def draw_network(self, activations=None):
        self.net_canvas.delete("all")
        size = self.layer_sizes()
        pos = self.node_positions(size)

        for indOflayer in range(len(pos)):
            for i in range(len(pos[indOflayer])):
                x, y = pos[indOflayer][i]
                if activations is not None:
                    color = self.act_color(self.check_Max(activations))
                else:
                    color = NODE_COLOR
                self.net_canvas.create_oval(x - 10, y - 10, x + 10, y + 10, fill=color, outline="white")
        for inOfLayer in range(len(pos) - 1):
            for i in range(len(pos[inOfLayer])):
                x, y = pos[inOfLayer][i]
                for j in range(len(pos[inOfLayer + 1])):
                    if activations is not None:
                        edColor = self.act_color(self.check_Max(activations))
                    else:
                        edColor = EDGE_ACTIVE_COLOR
                    x1, y1 = pos[inOfLayer + 1][j]
                    self.net_canvas.create_line(x, y, x1, y1, fill=edColor)
            ...
    ...
    def predict(self):
        ...
        print(type(activations), type(out))
